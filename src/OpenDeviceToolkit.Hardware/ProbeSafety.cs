...
public sealed record ProbeExecutionPlan(ProbeMode Mode, IReadOnlyList<Rp2040PinAssignment> Pins, ProbeElectricalState ElectricalState, IReadOnlyList<string> Actions);
...
public sealed class ProbeExecutionPlanner
{
    public ProbeExecutionPlan Prepare(ProbeModeProfile profile, IEnumerable<Rp2040PinAssignment> pins, double? targetVoltage)
    {
        ...
        return new(profile.Mode, pins.ToArray(), state, actions);
    }
}
