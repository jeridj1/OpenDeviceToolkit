...
public sealed class MainForm : Form
{
    ...
    public MainForm()
    {
        ...
        Controls.AddRange(new Control[] { 
            title, subtitle, 
            _scanButton, reportButton, workspaceButton, 
            _environmentButton, _deepScanButton, _usbScanButton,
            _voiceToggleButton, _researchButton, _rp2040Button, _eWasteModeCheckBox,
            _rebootButton, _bootloaderButton, _recoveryButton,
            _deviceSummary, _status, _output, 
            _voicePanel, _clarificationPanel
        });
    ...
    private void ToggleVoiceMode()
    {
        ...
        else
        {
            ...
            Speak("Voice mode enabled. Just talk naturally - I'll understand you.");
        }
        UpdateVoiceStatus();
    }
    ...
    private void OnCommandDetected(VoiceCommand command)
    {
        BeginInvoke(() => ProcessCommand(command));
    }
    ...
    private void OnClarificationNeeded(ClarificationRequest request)
    {
        BeginInvoke(() =>
        {
            ...
            _clarificationLabel.Text = request.Message;
            
            // Parse options from message
            var lines = request.Message.Split('\n');
            for (int i = 0; i < Math.Min(3, lines.Length - 1); i++)
            {
                var optionText = lines[i + 1].Trim().TrimStart('1', '2', '3', '.', ' ');
                var button = i == 0 ? _clarificationOption1 : i == 1 ? _clarificationOption2 : _clarificationOption3;
    ...
    private void HandleClarification(int optionNumber)
    {
        if (_pendingClarification == null)
            return;
        
        var response = optionNumber.ToString();
        var selection = ClarificationDialog.ParseClarificationResponse(response, 3);
        
        if (selection.HasValue)
        {
            // This would need to map to the actual command
    ...
    private void ProcessVoiceInput()
    {
        ...
        // Use the new intent detector with context
        var command = VoiceIntentDetector.DetectIntent(text, _speechService.Context);
        ProcessCommand(command);
    }

    private void ProcessCommand(VoiceCommand command)
    {
        _logger.Info($"Detected command: {command.Type}, Device: {command.Device}, Objective: {command.Objective}");
        ...
        switch (command.Type)
        {
            ...
            case VoiceCommandType.Help:
                Speak("You can say things like: scan my device, unlock this phone, generate a report, reboot it, or just describe what you want to do in your own words.");
                break;
            ...
            case VoiceCommandType.Unknown:
                ...
                {
                    Speak($"I'll help you with: {command.Objective}");
                    _ = StartResearchAsync(command.Objective);
                }
                else
                {
    ...
    {
        var status = _speechService.IsListening ? "Listening..." : 
                     _speechService.IsSpeaking ? "Speaking..." : 
                     "Ready";
    ...
    {
        _eWasteModeCheckBox.ForeColor = _eWasteMode ? Color.Red : SystemColors.ControlText;
    }
    ...
    private async Task StartResearchAsync(string? customObjective = null)
    {
        ...
        try
        {
            ...
            // Update context
            _speechService.Context.CurrentDevice = _device.Model;
            ...
            Speak($"Research complete. Found {hypotheses.Count} potential methods. Review results and use operation buttons.");
        }
        catch (Exception ex)
        {
    ...
    // ========== EXISTING METHODS (Updated with Voice Feedback) ==========
    ...
    private async Task ScanAsync()
    {
        ...
        try
        {
            ...
            Speak($"Device detected: {_device.Manufacturer} {_device.Model} running Android {_device.AndroidVersion}");
        }
        catch (Exception ex) { 
            _status.Text = $"Status\r\n------\r\nError: {ex.Message}"; 
            _logger.Error("Device scan failed.", ex); 
            Speak("Device scan failed");
        }
    ...
    private async Task RunOperationAsync(AndroidOperationKind kind)
    {
        ...
        try
        {
            ...
            Speak($"Operation {kind} executed successfully");
        }
        catch (Exception ex) { 
    ...
    private async Task RunEnvironmentCheckAsync()
    {
        ...
        try { 
            ...
            Speak($"Environment check complete. {failures} failures, {warnings} warnings.");
        }
        catch (Exception ex) { 
            _status.Text = $"Status\r\n------\r\nEnvironment check failed: {ex.Message}"; 
            _logger.Error("Environment check failed.", ex); 
            Speak("Environment check failed");
        }
        finally { SetActionButtons(true); SetOperationButtons(_device is not null); }
    }
    ...
    private async Task RunDeepScanAsync()
    {
        ...
        try
        {
            ...
            Speak("Running deep read-only scan");
            ...
            Speak($"Deep scan complete. {failed} failed probes, {ready} ready steps.");
        }
        catch (Exception ex) { 
            _status.Text = $"Status\r\n------\r\nDeep scan failed: {ex.Message}"; 
            _logger.Error("Deep Android scan failed.", ex); 
            Speak("Deep scan failed");
        }
        finally { SetActionButtons(true); SetOperationButtons(_device is not null); }
    }
    ...
    private void ShowUsbDevices()
    {
        try
        {
            ...
            Speak($"USB enumeration complete. Found {devices.Count} devices.");
        }
        catch (Exception ex)
        {
            _status.Text = $"Status\r\n------\r\nUSB enumeration failed: {ex.Message}";
            _logger.Error("USB enumeration failed.", ex);
            Speak("USB enumeration failed");
        }
    }
    ...
    private void OpenWorkspace()
    {
        ...
        catch (Exception ex) 
        {
            ...
            Speak("Failed to open workspace");
        }
    }
}
