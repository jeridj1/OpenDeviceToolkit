...
static void capture(uint pin, uint rate, uint words) {
    if (!valid_gpio((int)pin) || rate == 0 || words == 0 || words > CAPTURE_WORDS) {
        printf("ERR BAD_CAPTURE\n"); return;
    }
    all_high_z();
    if (!odt_logic_capture(pin, rate, capture_buffer, words)) {
...
static void handle_line(char *line) {
    if (!strcmp(line, "HELLO")) {
        printf("HELLO %s 1 SNIFF UART GPIO LOGIC_CAPTURE\n", ODT_VERSION);
    } else if (!strcmp(line, "SAFE")) {
        all_high_z(); printf("OK SAFE\n");
        ...
        if (sscanf(line + 8, "%u %u %u", &pin, &rate, &words) == 3) capture(pin, rate, words);
        else printf("ERR BAD_CAPTURE\n");
    } else {
...
int main(void) {
    stdio_init_all(); sleep_ms(1500); all_high_z();
    printf("ODT_READY %s\n", ODT_VERSION);
